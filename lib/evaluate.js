...
import { buildFullPathSegment, getComputedFlags, getTags, has } from '@bablr/agast-helpers/path';
...
function* parseStrategy(ctx, getState, rootURL, rootMatcher, rootValue) {
      ...
      m = yield buildCall(
        ...
        buildEmbeddedMatcher(
          buildPropertyMatcher(
            ...
            buildBoundNodeMatcher(
              [],
              buildTreeNodeMatcher(buildFragmentMatcher(buildNodeFlags(flags))),
            ),
          ),
        ),
        '    ',
  ...
  while (co) {
      ...
      switch (verb) {
        ...
        case 'shiftMatch': {
          ...
          if (isShift) {
            ...
            if (
              ...
              (embeddedMatcher.type === Matcher &&
                !getCooked(
                  get(
                    ['valueMatcher', 'nodeMatcher', 'open', 'type', 'content'],
          ...
          if (
            ...
            (embeddedMatcher.type === Node && embeddedMatcher.value.value.open.flags.token) ||
            ...
            const parsedResolvedMatcher = parsedPath || {
              type:
                (m.cover || !m.depth) &&
                !(m.node.value.open.flags.fragment && !m.node.value.open.flags.cover) &&
                !m.isNode
                  ? '_'
                  : '.',
              name: null,
            ...
            if (embeddedNodeMatcher.value.open.type === Symbol.for('NullNodeMatcher')) {
              ...
              returnValue = null;
              break;
            } else if (embeddedNodeMatcher.value.open.type === Symbol.for('GapNodeMatcher')) {
              if (parsedResolvedMatcher && effects.success === 'eat') {
                let { name } = parsedResolvedMatcher;
            ...
            const outerOptions = options;
            {
              ...
              if (
                (isNode || isCover) &&
                !s.referenceTagPath &&
                !(!s.depths.path && m.parent.nodeMatch.node.value.open.flags.token)
              ) {
                if (isShift) {
              ...
              let hasGap =
                ...
                (flags.token || mergedReference?.type === '@'
                  ? false
                  : m.parent.node.value.open.flags.hasGap &&
                    !(mergedReference.flags.intrinsic && nodeMatch.node.value.open.flags.token));
            ...
            if (isLiteral || shouldInterpolate) {
              let returnNode = Tags.getAt(-1, m.node.value.children).value.node;
              ...
              if ([CloseNodeTag, NullTag, GapTag].includes(prevTagPath?.tag.type)) {
                prevTagPath = m.path.parent ? m.path.parent.tagPathAt(-1) : m.path.tagPathAt(0);
              }
        ...
        case 'exec': {
          ...
          let resolvedLanguage = resolveLanguage(s.languages, bindingMatcher.segments);
...

    {
      ...
      let isEmpty_ = isEmpty(streamFromTree(m.node));
