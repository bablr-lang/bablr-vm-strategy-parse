...
function* parseStrategy(ctx, getState, rootLanguage, rootMatcher, rootValue) {
...

  {
    ...
    yield buildCall('init', matcher.bindingMatchers);
  ...
  while (co) {
    ...
    instrLoop: for (;;) {
      ...
      switch (verb) {
        ...
        case 'eatHeld': {
          ...
          if (
            ...
            const parsedResolvedMatcher = parsedPath || {
              type:
                (m.getPublic().getCover() || !m.getPublic().depth) &&
                m.getPublic().getNode().value.type !== Symbol.for('__') &&
                !m.getPublic().isNode
                  ? '_'
                  : '.',
              name: null,
                    ...
                    yield buildCall(
                      ...
                      buildEmbeddedTag(
                        buildChild(
                          ...
                          m.getPublic().isNode
                            ? ownReference
                            : mergeReferences(m.getPublic().mergedReference, ownReference),
                        ),
                      ),
                    );
            ...
            const isCoverBoundary =
              ...
              (isShift
                ? true
                : m.getPublic().getParent()
                ? m.getPublic().isNode || !m.getPublic().getCover()
                : true);
            ...
            const shouldInterpolate =
              ...
              !isShift &&
              (!m.getPublic().getCover()?.didShift || m?.isNode) &&
              (isNode || isCover) &&
              !(
                mergedReference.flags.intrinsic ||
                (!m.getPublic().isNode &&
                  m.getPublic().getCoveredBoundary().mergedReference.flags.intrinsic)
              ) &&
              !['#', '@'].includes(mergedReference.type) &&
              !options?.suppressGap;

            if (isShift && !(m.getPublic().isNode || m.getPublic().isCover)) {
              throw new Error('shift must be returned from a node or cover production');
            }

            let literalResult;

            if (isShift && finishedMatch.getPublic().getState().status === 'rejected') {
              if (
            ...
            if (
              isShift &&
              m.getPublic().getCover() &&
              !m.getPublic().getCover().mergedReference.flags.expression
            ) {
            ...
            const outerOptions = options;
            {
              ...
              if (
                ...
                !(
                  s.depths.path === 0 &&
                  m.getPublic().getParent()?.getNodeMatch().getNode().value.flags.token
                )
              ) {
                if (isShift) {
              ...
              let nodeMatch = m.getPublic().getParent()
                ? m.getPublic().getParent().getNodeMatch()
                : m;
              let hasGap =
                gapsAllowed &&
            ...
            if (isLiteral || shouldInterpolate) {
              let returnNode = Tags.getAt(-1, m.getPublic().getNode().value.children).value.node;
        ...
        case 'startSpan': {
          let { 0: name, 1: guard } = args;
          yield buildCall(verb, name, guard);
          break;
        }
...

    {
      ...
      const { isNode, effects, allowEmpty, isCover } = m.getPublic();
      ...
      if (
        coroutines.has(m) &&
        (isNode || isCover || (m.getPublic().depth === 0 && m.getPublic().matcher.type !== '__')) &&
        !isThrowing
      ) {
        yield buildCall('advance', buildEmbeddedTag(buildCloseNodeTag()));
      }

      if (finishedMatch.getPublic().options.shift && finishedCo.value?.shift && coroutines.has(m)) {
      ...
      let node = m.getPublic().getNode();

      let isEmpty_ =
        !isThrowing &&
        isEmpty(
          streamFromTree(finishedMatch.getPublic().getNode(), { getGapNode, checkBalance: false }),
        );
      ...
      node = finishedMatch.getPublic().getNode();

      if (finishedMatch.getPublic().depth === 0) {
        if (!throwing && node) {
          return node;
        } else {
          throw new Error(
            `parse failed after ${finishedMatch.getPublic().state.sourceIndex} characters`,
          );
        }
      }
      ...
      let returnNode = !failing
        ? finishedMatch.getPublic().getNode()
        : finishedMatch.getPublic().options.internal &&
          finishedMatch.getPublic().getShiftMatch() &&
          finishedMatch.getPublic().isCoverBoundary &&
          finishedMatch.getPublic().effects.failure === 'none'
        ? finishedMatch.getPublic().getShiftMatch().getNode()
        : null;
      ...
      matchReturnValue =
        ...
        freeze({
          node: returnNode,
          value: finishedMatch.getPublic().options.shift
            ? finishedCo.value?.value
            : finishedCo.value,
        });
      continue;
    }
