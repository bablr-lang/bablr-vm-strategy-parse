      ...
      switch (verb) {
        ...
        case 'shiftMatch': {
          ...
          if (
            ...
            const parsedResolvedPath = !parsedPath
              ? parsedPath || {
                  type: '.',
                  name: null,
                  isArray: false,
            ...
            let mergedMatcher =
              (!m.isNode && !m.isCover && ownReference.type === '.'
                ? mergeReferences(m.mergedReference, ownReference)
                : ownReference) || buildReference('.');
            ...
            if (isLiteral || shouldInterpolate) {
              ...
              returnValue = new NodeFacade(m.node.tags.at(-1, -1).value.node);
