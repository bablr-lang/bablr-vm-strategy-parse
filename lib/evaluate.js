        ...
        case 'shiftMatch': {
          ...
          if (
            ...
            const parsedResolvedMatcher = parsedPath || {
              type:
                (m.cover || !m.depth) && m.node.value.type !== Symbol.for('__') && !m.isNode
                  ? '_'
                  : '.',
              name: null,
              isArray: false,
            ...
            const shouldInterpolate =
              ...
              (isNode || isCover) &&
              !(
                mergedReference.flags.intrinsic || m.coveredBoundary.mergedReference.flags.intrinsic
              ) &&
            ...
            const outerOptions = options;
            {
              ...
              if (
                (isNode || isCover) &&
                // !s.resultPath?.referenceTagPath &&
                !(s.depths.path === 0 && m.parent?.nodeMatch.node.value.flags.token)
              ) {
                if (isShift) {
              ...
              let nodeMatch = m.parent ? m.parent.nodeMatch : m;
              let hasGap =
                gapsAllowed &&
                (flags.token || mergedReference?.type === '@'
                  ? false
                  : nodeMatch.node.value.flags.hasGap &&
