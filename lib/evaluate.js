      ...
      switch (verb) {
        ...
        case 'shiftMatch': {
          ...
          if (
            ...
            if (!get('open', embeddedNodeMatcher)) {
              embeddedMatcher = buildEmbeddedMatcher(get(['children', 0], embeddedNodeMatcher));
              embeddedNodeMatcher = get(['valueMatcher', 'nodeMatcher'], embeddedMatcher.value);
            }
            ...
            const parsedResolvedMatcher = parsedPath || {
              type:
                (m.cover || !m.depth) &&
                !(m.node.flags.fragment && !m.node.flags.cover) &&
                !m.isNode
                  ? '_'
                  : '.',
              name: null,
              ...
              if (mergedReference && mergedReference.name !== '.') {
                ...
                if (
                  ...
                  ((name && !s.node.has(name)) ||
                    (refType === '_' && !s.node.has(buildFullPathSegment('_'))))
                ) {
                  if (name) {
                    if (options.bind) {
                      yield buildCall(
            ...
            const outerOptions = options;
            {
              ...
              const options = {
                shift: mergedReference.flags.expression && (outerOptions.shift ?? true),
              ...
              let hasGap =
                ...
                (flags.token || mergedReference?.type === '@'
                  ? false
                  : m.parent.node.flags.hasGap &&
                    !(mergedReference.flags.intrinsic && nodeMatch.node.flags.token));
            ...
            if (isLiteral || shouldInterpolate) {
              let returnNode = Tags.getAt(-1, m.node.children.at(-1).value.tags).value.node;
