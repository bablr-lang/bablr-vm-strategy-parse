...
import { deepFreezeRecord, freezeRecord, isObject } from '@bablr/agast-helpers/object';
...
import { buildReference, buildTag, parseTagType } from '@bablr/agast-helpers/builders';
    ...
    if (!flags.token) {
      m = yield buildCall(
        ...
        buildEmbeddedCallable({
          reference: null,
          bindings: [],
          nodeMatcher: buildTag(TreeNodeMatcher, {
            flags,
            ...
            let outerOptions = options;
            {
              ...
              if (!shouldInterpolate) {
                m = yield buildCall(
                  'call',
                  Symbol.for(verb),
                  buildEmbeddedCallable(deepFreezeRecord({ reference, bindings, nodeMatcher })),
              ...
              if (shouldInterpolate) {
                ...
                if (resolvedRef.type === '@') {
                  staticAttributes = deepFreezeRecord({ ...staticAttributes, cooked: undefined });
                }
                ...
                start = buildOpenNodeTag(
                  ...
                  name,
                  cookedLiteral,
                  deepFreezeRecord({ ...staticAttributes, ...attributes }),
                  cookedLiteral != null,
                );
              } else if (isCover) {
