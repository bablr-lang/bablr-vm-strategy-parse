  ...
  while (co) {
      ...
      switch (verb) {
        ...
        case 'holdForMatch': {
          ...
          if (
            ...
            const isCoverBoundary = (isNode || isCover) && (isHold ? !m.parent.cover : !m.cover);
            ...
            if (
              ...
              if (isHold) {
                let index = s.depths.shift + 1;
                let height = s.depths.nodeShift + 1;
...

    {
      ...
      if ((isNode || finishedMatch.depth === 0) && !isThrowing) {
        ...
        if (depth) {
          let languagePath = unresolveLanguage(ctx, m.parent.language, m.language.canonicalURL);
