...
const fragmentMatcher = buildPropertyMatcher(
  null,
  null,
  buildBasicNodeMatcher(buildFragmentMatcher()),
);
      ...
      switch (verb) {
        ...
        case 'shiftMatch': {
          ...
          if (
            ...
            const isCoverBoundary = (isNode || isCover) && (isShift ? true : !m.cover);
            ...
            mergedReference = buildReferenceTag(
              mergedReference.value.type,
              mergedReference.value.name,
              mergedReference.value.isArray,
              {
                ...mergedReference.value.flags,
                hasGap: gapsAllowed && mergedReference.value.flags.hasGap,
              },
            );
            ...
            const shouldInterpolate =
              ...
              !m.cover?.didShift &&
              (isNode || isCover) &&
              (parsedResolvedPath.flags.hasGap || mergedReference.value.flags.hasGap) &&
              !options?.suppressGap;

            if (flags.cover && m.cover && !isShift) throw new Error();
            ...
            if (isShift && m.cover && !m.cover.mergedReference.value.flags.expression) {
            ...
            if (s.referenceTagPath?.tag.value.name) {
              let { type: mType, name: mName } = mergedReference.value;
              let { type: oType, name: oName } = ownReference.value;
              if (
                mType !== '.' &&
                oType !== '.' &&
