...
function* parseStrategy(ctx, rootURL, rootMatcher, rootValue) {
  ...
  while (co) {
...

    {
      ...
      const { isNode, effects, allowEmpty, isCover, isCoverBoundary } = m;
      ...
      if ((isNode || isCover || finishedMatch.depth === 0) && !isThrowing) {
        yield buildCall('advance', buildEmbeddedTag(buildCloseNodeTag()));
      }
      ...
      let isEmpty_ = isEmpty(allTagsFor([m.path.tagPathAt(0), m.path.tagPathAt(-1)]));
      ...
      matchReturnValue = failing
        ? finishedMatch.isCoverBoundary && finishedMatch.shiftMatch
