        ...
        case 'shiftMatch': {
          ...
          if (
            ...
            const parsedResolvedMatcher = parsedPath || {
              type:
                (m.cover || !m.depth) &&
                !(m.node.value.flags.fragment && !m.node.value.flags.cover) &&
                !m.isNode
                  ? '_'
                  : '.',
              name: null,
            ...
            if (embeddedNodeMatcher.value.type === Symbol.for('NullNodeMatcher')) {
              ...
              returnValue = null;
              break;
            } else if (embeddedNodeMatcher.value.type === Symbol.for('GapNodeMatcher')) {
              if (parsedResolvedMatcher && effects.success === 'eat') {
                let { name } = parsedResolvedMatcher;
            ...
            const outerOptions = options;
            {
              ...
              if (
                (isNode || isCover) &&
                !s.referenceTagPath &&
                !(!s.depths.path && m.parent.nodeMatch.node.value.flags.token)
              ) {
                if (isShift) {
              ...
              let hasGap =
                ...
                (flags.token || mergedReference?.type === '@'
                  ? false
                  : m.parent.node.value.flags.hasGap &&
                    !(mergedReference.flags.intrinsic && nodeMatch.node.value.flags.token));
