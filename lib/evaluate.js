...
function* parseStrategy(ctx, rootMatcher, rootValue) {
  ...
  while (co) {
        ...
        case 'holdForMatch': {
          ...
          if (
            ...
            if (isArray(nodeMatcher)) {
              // initializer
              const { name, type, isArray, flags } = parsedResolvedPath;
              ...
              if (type === '.' || !s.node.has(name)) {
                let ownReference = buildReferenceTag(type, name, isArray, flags);
                yield buildCall(
                  'advance',
                  buildEmbeddedTag(
              ...
              if (parsedResolvedPath && effects.success === 'eat') {
                const { type, name, isArray, flags } = parsedResolvedPath;
            ...
            // advance reference?
            if (!isHold && isCoverBoundary && effects.success === 'eat' && !s.referencePath) {
              const { type, name, isArray, flags } = mergedReference.value;

              if (isArray && ((name && !s.node.has(name)) || (type === '.' && !s.node.hasRoot()))) {
                yield buildCall(
                  'advance',
                  buildEmbeddedTag(buildReferenceTag(type, name, isArray, flags)),
                );
                yield buildCall('advance', buildEmbeddedTag(buildInitializerTag(true)));
              }
...

    {
      ...
      let isEmpty_ = isEmpty(allTagsFor(isNode ? [TagPath.from(m.innerPath, 0), null] : m.range));
