...
import { has, list, TagPath } from '@bablr/agast-helpers/path';
    ...
    instrLoop: for (;;) {
      ...
      switch (verb) {
        ...
        case 'shiftMatch': {
          ...
          if (
            ...
            for (let bindingMatcher of bindingMatchers) {
              ...
              languages = BList.push(languages, language);
            }
            ...
            const canInterpolate =
              (s.atGap || (s.shifted && options.held === 'eat')) &&
              (isNode || isCover) &&
        ...
        case 'eatHeld': {
          ...
          if (matcher) {
            let embeddedNodeMatcher = get(['valueMatcher', 'nodeMatcher'], matcher.value);
            if (embeddedNodeMatcher.value.name !== Symbol.for('GapNodeMatcher')) throw new Error();

            let refMatcher_ = get('refMatcher', matcher.value);
            if (refMatcher_) {
              const refMatcher = reifyExpression(refMatcher_);
              const resolvedRefMatcher = refMatcher || {
                type:
                  (m.getCover() || !m.depth) &&
                  m.getNode().value.type !== Symbol.for('__') &&
              ...
              yield buildCall(
                'advance',
                buildEmbeddedTag(
                  buildChild(ReferenceTag, referenceFromMatcher(resolvedRefMatcher)),
                ),
              );
            }
          }
        ...
        case 'defineAttribute': {
          ...
          yield buildCall('advance', buildEmbeddedTag(buildAttributeDefinitionTag(path, value)));
          break;
        }
