...
import { buildFullPathSegment, getTags, has, list, TagPath } from '@bablr/agast-helpers/path';
      ...
      switch (verb) {
        ...
        case 'eatHeld': {
          ...
          if (matcher) {
            ...
            if (refMatcher) {
              const parsedPath = reifyExpression(refMatcher);
              const parsedResolvedMatcher = parsedPath || {
                type:
                  (m.getCover() || !m.depth) &&
                  m.getNode().value.type !== Symbol.for('__') &&
              ...
              yield buildCall(
                'advance',
                buildEmbeddedTag(
                  buildChild(ReferenceTag, referenceFromMatcher(parsedResolvedMatcher)),
                ),
              );
            }
          }
