...
import { buildCall, buildEmbeddedMatcher } from '@bablr/agast-vm-helpers/builders';
import { EmbeddedObject } from '@bablr/agast-vm-helpers/symbols';
import { isPlainObject } from '@bablr/helpers/object';
import { buildEmbeddedNode } from '@bablr/agast-helpers/builders';

const { freeze } = Object;

export const coroutines = new WeakMap();

function* defaultFragment({ props: { productionName } }) {
  yield buildCall(
    'eat',
...
export const buildCoroutine = (ctx, s, m, value, intrinsicValue = null) => {
  ...
  const { propertyMatcher, language, isCover, cover, isCoverBoundary } = m;
  ...
  const args = freeze({
    ...
    flags,
    isCover,
    isCovered: cover && !isCoverBoundary,
    isCoverBoundary,
    grammar,
    context: ctx,
    ctx,
    attributes,
    attrs: attributes,
    intrinsicValue: intrinsicValue && buildEmbeddedNode(intrinsicValue),
  });
